...
let typingOn = (io) => {
	...
	io.on("connection", (socket) => { //lang nghe su kien connection(danh cho nguoi dung khi truy cap vao trang web cua minh)
		...
		// console.log("---------------------------------------------------------------------------")
		socket.on("user-is-typing", (data) => { //lang nghe sk tu clien voi ten ...va gia tri keo len ...
			// console.log(data);
			...
			if (data.groupId) {
				...
				if (clients[data.groupId]) {
					emitNotifyToArray(clients, data.groupId, io, "response-user-is-typing", respone);
				}
			...
			if (data.contactId) {
				...
				if (clients[data.contactId]) {
					emitNotifyToArray(clients, data.contactId, io, "response-user-is-typing", respone);
				}
